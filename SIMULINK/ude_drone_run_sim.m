%% UDE Drone Parameters

%% Model

open_system('ude_drone')
...
open_system('ude_drone/Quadcopter','force')

%% Body Subsystem

open_system('ude_drone/Quadcopter/Body','force')

%% Motors Subsystem

open_system('ude_drone/Quadcopter/Electrical','force')

%% Maneuver Controller

set_param('ude_drone/Maneuver Controller','open','on');
...
for i = 1:N_RUNS
    ...
    % A. Randomize Wind (0 to 12.5 m/s)
...
% Plot 1: Trend Analysis (Wind vs Improvement)
subplot(1, 3, 1);
...
% GRAPH 3: Energy Efficiency vs Error
subplot(1, 3, 3);
...
if iae_improvement > 0
    fprintf('   Reduced cumulative tracking error (IAE) by %.1f%%, eliminating steady-state drift."\n', iae_improvement);
else
    fprintf('   (No improvement found)\n');
end
fprintf('-------------------------------------------------\n');

fprintf('2. DISTURBANCE REJECTION:\n');
fprintf('   PID allows %.3f meters error per 1 m/s of wind.\n', dr_ratio_pid);
fprintf('   UDE allows %.3f meters error per 1 m/s of wind.\n', dr_ratio_ude);
fprintf('   Disturbance Rejection Ratio of %.3f m/(m/s), effectively insulating flight path from wind shear."\n', dr_ratio_ude);
fprintf('=================================================\n');
...
if safety_improvement > 0
    fprintf('   "Reduced critical worst-case positioning error by %.1f%%."\n', safety_improvement);
else
...
if sensitivity_reduction > 0
    fprintf('   "Lowered sensitivity to wind disturbances by %.1f%%, decoupling performance from weather conditions."\n', sensitivity_reduction);
else
...
if ude_crash_rate < pid_crash_rate
    fprintf('   "Eliminated system instability events, reducing crash probability from %.1f%% to %.1f%%."\n', pid_crash_rate, ude_crash_rate);
